<x-layout>
    ...
    <div class="flex items-center justify-center mt-6">
        <div class="text-center">
            <img src="{{ asset($arrendamiento->image) }}" alt="Lease photo" class="mx-auto my-4 rounded-lg shadow-xl" width="300" height="300">
        ...
        <div class="flex justify-around mt-[-300px]">
            ...
            <div class="text-left shadow-xl rounded-[10px] p-8">
                ...
                @foreach ($arrendamiento->conditions as $condition)
                    <p class="text-lg font-bold text-left">
                        {{ $condition->name }}: <span class="text-[#BF7534]">{{ $condition->pivot->answer == 0 ? 'Allowed' : 'Not allowed' }}</span>
                    </p>
                @endforeach
            </div>
            ...
            <div class="text-left shadow-xl rounded-[10px] px-[20px] py-[10px]">
                ...
                @foreach ($arrendamiento->services as $service)
                    <p class="text-lg font-bold text-left">
                        {{ $service->name }}: <span class="text-[#BF7534]">{{ $service->pivot->answer == 0 ? 'Yes' : 'No' }}</span>
                    </p>
                @endforeach
            </div>
        </div>
